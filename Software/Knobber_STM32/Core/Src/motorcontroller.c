...
void initMotorControl()
{
	...
	AS5600_init(&hi2c3, AS5600_CONF_L_HYST_OFF | AS5600_CONF_L_OUTS_AN | AS5600_CONF_L_PM_NOM | AS5600_CONF_L_PWMF_115, AS5600_CONF_H_FTH_SLOW | AS5600_CONF_H_SF_2x | AS5600_CONF_L_HYST_OFF);
...
void update()
{
	HAL_I2C_Mem_Read_IT(&hi2c3, AS5600_I2C_ADDR, AS5600_REG_RAWANGLE, I2C_MEMADD_SIZE_8BIT, (uint8_t*)moco.as5600_i2c_angle, 2);
}
...
void updateIndent()
{
	...
	if ((moco.old_position_error > 10) && ( moco.position_error < -10))
	{
		// jump forward to new indent
		moco.indent_increments += 1;
	}
	else if ((moco.old_position_error < -10) && ( moco.position_error > 10))
	{
		// jump backwards to new indent
		moco.indent_increments -= 1;
	}
...
void calibrateOffset(uint8_t calibration_power)
{
	...
	while (AS5600_getRawAngle(&hi2c3) > 2048/7)
	{
		angle += 5;
	...
	{
		moco.encoder_offset += AS5600_getRawAngle(&hi2c3);
	}
	moco.power = 0;
	updatePosition(0);
